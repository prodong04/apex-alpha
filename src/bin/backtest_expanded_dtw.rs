...
fn main() {
    ...
    println!("📂 Loading expanded 60-coin universe datasets from {}...", data_dir);
    let (symbols, timestamps, price_series) = load_klines(data_dir);
    let total_bars = timestamps.len();

    println!("✅ Synchronized {} symbols across {} 1m bars (~{:.1} days)",
    ...
    println!("\n🚀 Running 60-Coin DTW Momentum Spillover Simulation...");
    ...
    for bar_idx in lookback_bars..(total_bars - max_holding_bars) {
        ...
        // BTC Trend Filter: Skip if BTC is dumping (> -0.4% in 15m)
...
fn print_results(trades: &[TradeRecord], num_symbols: usize, total_bars: usize) {
    println!("\n==================================================================");
    println!("💎 EXPANDED {}-COIN DTW MOMENTUM SPILLOVER REPORT", num_symbols);
    println!("==================================================================");
    println!("• Universe Size       : {} Liquid Tokens", num_symbols);
    println!("• Dataset Range       : {:.1} Days ({} 1-min bars)", (total_bars as f64)/1440.0, total_bars);
    println!("• Total Basket Trades : {} trades (vs 18 in 19-coin pilot)", trades.len());
    ...
    println!("\n🔍 SAMPLE EXPANDED SPILLOVER BASKET TRADES (최근 15건 샘플):");
    println!("| 진입 일시 (UTC) | 리더 코인 (5분 돌파) | DTW 유사 래거 매수 바스켓 | 보유 시간 | 순익 ($) | 청산 유형 |");
    println!("| :--- | :--- | :--- | :---: | :---: | :--- |");
    for t in trades.iter().rev().take(15) {
        let dt = DateTime::<Utc>::from_timestamp_millis(t.entry_ts)
            .map(|d| d.format("%m-%d %H:%M").to_string())
            .unwrap_or_default();
        let laggards_str = t.laggards.join(" + ");
        println!("| {} | {:<12} (+{:.2}%) | {:<28} | {:>2}분 | ${:>6.2} | {:<12} |",
