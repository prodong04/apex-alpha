...
fn main() {
    ...
    println!("\n🏛️ STARTING HIGH-CONVICTION DTW STAT-ARB ENGINE (2022-2025)");
    println!("• Regime Filters: Volume Z-Score >= 3.0σ (Institutional Breakout) + Spread Z-Score >= 2.5σ");
    println!("• Execution Model: Market Beta = 0.00 (50% Short Leader / 50% Long DTW Cohort)");
    println!("• Realistic Friction: VIP Taker 10 bps roundtrip per leg + 3 bps slippage");
    println!("• Validation: 10% Isolated Hold-out split against P-Hacking\n");
    ...
    let leader_window = 5;
    let min_volume_z = 2.5; // Institutional Volume Spike Z >= 2.5σ
    let min_spread_z = 2.2; // Statistical Divergence Spread Z >= 2.2σ
    let target_spread_convergence = 0.018; // +1.8% Target Take-Profit
    let stop_loss_spread = -0.012; // -1.2% Risk-Managed Stop Loss
    let max_holding = 40; // 40 mins maximum holding
    let base_fee_rate = 0.0005; // 5 bps VIP Taker Fee per trade leg
    ...
    let total_sim_bars = total_bars - max_holding - (lookback_bars + pit_rebalance_interval);
    let log_interval = total_sim_bars / 20; // Log progress every 5%
    let mut last_log_bar = lookback_bars + pit_rebalance_interval;

    for bar_idx in ((lookback_bars + pit_rebalance_interval)..total_bars - max_holding).step_by(step) {
        let ts = timestamps[bar_idx];

        // ⏱️ Mandatory ETA & Progress Logging (Every 5%)
        if bar_idx - last_log_bar >= log_interval {
            let processed = bar_idx - (lookback_bars + pit_rebalance_interval);
        ...
        // 3. High-Conviction Stat-Arb Signal Generation
        ...
        for basket in &current_baskets {
            ...
            // Find Leader with significant Volume Z-Score and Price Surge
            let mut overshooter = None;
            let mut best_vol_z = 0.0;
            ...
            for s in &basket.symbols {
                if let (Some(curr), Some(prev)) = (current_bar_map.get(s), prev_bar_map.get(s)) {
                    ...
                    if ret >= 0.012 { // Min +1.2% move
                        // Compute 60-bar Rolling Volume Z-Score
                        ...
                        if vol_history.len() >= 30 {
                            ...
                            if vol_z >= min_volume_z && ret > max_overshoot {
                                max_overshoot = ret;
                                best_vol_z = vol_z;
            ...
            if let Some(short_sym) = overshooter {
                ...
                let mut max_spread_z = 0.0;

                for (s_idx, s) in basket.symbols.iter().enumerate() {
                    if s == &short_sym { continue; }
                    if let (Some(curr), Some(prev)) = (current_bar_map.get(s), prev_bar_map.get(s)) {
                        let ret = (curr.close - prev.close) / prev.close;
                        let spread_diff = max_overshoot - ret;

                        if spread_diff >= 0.010 {
                            // Compute Spread historical standard deviation over last 60 bars
                            let mut spread_history = Vec::with_capacity(60);
                            for past_i in (bar_idx - 60)..bar_idx {
                                let p_ts = timestamps[past_i];
                                if let (Some(b1), Some(b2)) = (
                                    timeline_bars.get(&p_ts).and_then(|m| m.get(&short_sym)),
                                    timeline_bars.get(&p_ts).and_then(|m| m.get(s)),
                                ) {
                                    spread_history.push(b1.close / b2.close);
                                }
                            }

                            if spread_history.len() >= 30 {
                                let mean_sp = spread_history.iter().sum::<f64>() / spread_history.len() as f64;
                                let var_sp = spread_history.iter().map(|sp| (sp - mean_sp).powi(2)).sum::<f64>() / (spread_history.len() - 1) as f64;
                                let std_sp = var_sp.sqrt();
                                let current_sp = curr.close / prev.close; // relative ratio
                                let spread_z = if std_sp > 1e-6 { (current_sp - mean_sp).abs() / std_sp } else { 0.0 };
                ...
                if let Some(long_sym) = best_hedge {
                    ...
                    let mut exit_reason = "TIMEOUT_40M".to_string();
...
fn print_stat_arb_tear_sheet(trades: &[StatArbTrade]) {
    println!("\n==================================================================");
    println!("🏛️ TWO SIGMA TEAR SHEET: HIGH-CONVICTION DTW STAT-ARB (2022-2025)");
    println!("==================================================================");
    println!("• Total High-Conviction Trades : {} trades", trades.len());
    ...
    println!("\n📅 2. YEAR-BY-YEAR HIGH-CONVICTION STAT-ARB PERFORMANCE:");
    ...
    for (yr, trs) in &year_map {
        ...
        println!("| {:>4}년 | {:>4}회 | {:>6.1}% | ${:>11.2} | {:>10.2} | {:>6.1}분 | High-Conviction Stat-Arb |",
    ...
    println!("🏆 FINAL CONSOLIDATED HIGH-CONVICTION AUDIT:");
