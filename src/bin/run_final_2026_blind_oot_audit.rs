...
fn load_full_5year_vision_data(csv_dir: &str) -> (Vec<String>, BTreeMap<i64, HashMap<String, BarData>>) {
    println!("📂 Indexing 14GB of 2022-2026 (Full 5-Year including 2026 OOT) Point-In-Time 1-minute historical datasets...");
    ...
    for entry in entries {
        if let Ok(e) = entry {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("csv") {
                ...
                // 🔓 2026 Unlocked for Final Blind Out-of-Time Audit!
...
fn run_portfolio_audit(
    ...
    println!("\n🏛️ EXECUTING 5-YEAR FULL WALK-FORWARD AUDIT: [{}]", fee_mode);
    println!("• Unlocked 2026 Blind OOT Set | Total Timeline: 2022.01 ~ 2026.08");
    println!("• Book Limit: Max {} Concurrent Pairs ({} Longs + {} Shorts)", max_slots, max_slots, max_slots);
    println!("• Net Portfolio Beta: 0.00 | Capital per Pair: ${:.0} ($10k Total Book)\n", slot_capital);
    ...
    for bar_idx in start_bar..(total_bars - max_holding) {
        ...
        if bar_idx - last_log_bar >= log_interval {
            ...
            println!(
                "  [{}] {:>5.1}% ({:>7} / {:>7} bars) | Elapsed: {:>02}m{:>02}s | ETA: {:>02}m{:>02}s | Speed: {:>6.0} b/s | Closed: {}",
                fee_mode, pct, processed, total_sim_bars, elapsed_min, elapsed_rem_sec, eta_min, eta_rem_sec, speed, closed_trades.len()
            );
    ...
    print_final_5year_tear_sheet(fee_mode, &closed_trades, &daily_equity, initial_capital);
}

fn print_final_5year_tear_sheet(
    title: &str,
    trades: &[ClosedTrade],
    ...
    println!("\n==================================================================");
    println!("🏛️ TWO SIGMA FINAL 5-YEAR TEAR SHEET: {}", title);
    println!("==================================================================");
    println!("• Total 5-Year High-Conviction Trades : {} trades (2022.01 ~ 2026.08)", trades.len());
    ...
    println!("------------------------------------------------------------------");
    println!("🏆 FINAL 5-YEAR CONSOLIDATED INSTITUTIONAL AUDIT (2022 ~ 2026):");
    ...
    // Export full 5-year daily equity JSON
    ...
    if let Ok(f) = std::fs::File::create(json_path) {
        ...
        println!("💾 Full 5-Year daily equity exported to: {}", json_path);
    }
}
...
fn main() {
    ...
    // Run Full 5-Year Walk-Forward Audit
    run_portfolio_audit(
        &timeline_bars,
        &timestamps,
        "FINAL 5-YEAR (2022-2026) TOP-5 STAT-ARB PORTFOLIO BOOK",
        0.0000, // 0 bps Maker Post-Only
