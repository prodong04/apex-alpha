...
fn simulate_engine(
    ...
    slippage_bps: f64,
    min_surge_ret: f64,
    min_spread_divergence: f64,
    target_spread_convergence: f64,
    stop_loss_spread: f64,
    max_holding: usize,
    dtw_quantile_cutoff: f64,
) -> Vec<StatArbTrade> {
    ...
    println!("\n▶️ RUNNING SIMULATION: [{}]", fee_model_name);
    println!("• Surge Threshold: +{:.1}% | Spread Divergence: +{:.1}% | Target Profit: +{:.1}%",
             min_surge_ret * 100.0, min_spread_divergence * 100.0, target_spread_convergence * 100.0);
    println!("• Fee per leg: {:.2} bps | Slippage: {:.2} bps", base_fee_rate * 10000.0, slippage_bps * 10000.0);
    ...
    for bar_idx in (start_bar..(total_bars - max_holding)).step_by(step) {
        ...
        if bar_idx - last_log_bar >= log_interval {
            ...
            println!(
                "  [{}] {:>5.1}% | Elapsed: {:>02}m{:>02}s | ETA: {:>02}m{:>02}s | Speed: {:>6.0} b/s | Trades: {}",
                fee_model_name, pct, elapsed_min, elapsed_rem_sec, eta_min, eta_rem_sec, speed, trades.len()
            );
            last_log_bar = bar_idx;
        }

        // 1. Weekly PIT Universe
                        ...
                        if vol_z >= 2.5 {
                            if let Some(cohort_list) = current_top_dtw_pairs.get(s) {
                                ...
                                if let Some(long_sym) = best_cohort {
                                    ...
                                    let mut exit_reason = format!("TIMEOUT_{}M", max_holding);
                                    ...
                                    for forward_bar in (bar_idx + 1)..=(bar_idx + max_holding) {
                                        ...
                                        if let (Some(s_bar), Some(l_bar)) = (fwd_map.get(&short_sym), fwd_map.get(&long_sym)) {
                                            ...
                                            // Dynamic early profit take on convergence
...
fn print_detailed_tear_sheet(title: &str, trades: &[StatArbTrade]) {
    println!("\n==================================================================");
    ...
    println!("🏆 FINAL CONSOLIDATED AUDIT:");
...
fn main() {
    ...
    // Model 1: High-Conviction Large Divergence (Taker 10 bps fee + 3 bps slippage)
    let trades_taker = simulate_engine(
        &timeline_bars,
        &timestamps,
        "MODEL 1: HIGH-CONVICTION TAKER (Surge >= 2.2%, Divergence >= 1.8%)",
        0.0005, // 5 bps VIP Taker per leg (10 bps roundtrip)
        0.0003, // 3 bps slippage
        0.022,  // +2.2% Price Surge
        0.018,  // +1.8% Spread Divergence
        0.025,  // +2.5% Target Profit
        -0.015, // -1.5% Stop Loss
        30,     // 30 mins holding
        0.05,   // Top 5% Closest DTW Pairs
    );
    print_detailed_tear_sheet("120H HIGH-CONVICTION TAKER MODEL", &trades_taker);

    // Model 2: Institutional Maker Post-Only (0 bps Maker fee)
    let trades_maker = simulate_engine(
        &timeline_bars,
        &timestamps,
        "MODEL 2: INSTITUTIONAL MAKER POST-ONLY (0 bps Maker Fee)",
        0.0000, // 0 bps Maker VIP
        0.0000, // 0 bps Slippage on Limit Post-Only
        0.015,  // +1.5% Price Surge
        0.012,  // +1.2% Spread Divergence
        0.018,  // +1.8% Target Profit
        -0.010, // -1.0% Stop Loss
        45,     // 45 mins holding
        0.08,   // Top 8% Closest DTW Pairs
    );
    print_detailed_tear_sheet("120H INSTITUTIONAL MAKER POST-ONLY MODEL", &trades_maker);
}
