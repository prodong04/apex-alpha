...
fn main() {
    ...
    let svg_path = "/Users/elias/apex-alpha/scratch/ivory_strictly_clipped.svg";
    generate_strictly_clipped_svg(&records, &hourly_pnl, svg_path);
    println!("✅ Generated Strictly Clipped Ivory SVG");
}

fn generate_strictly_clipped_svg(
    records: &[TradeRecord],
    ...
    svg.push_str(&format!(
        ...
        <defs>
            ...
            <!-- Clip path strictly locking all chart drawings inside the white card rounded rect! -->
        ...
        "##,
        w = width, h = height, cx = card_x, cy = card_y, cw = card_w, ch = card_h
    ));

    // 1. TOP HEADER (y: 45..120)
    svg.push_str(r##"
        <g transform="translate(50, 42)">
            ...
            <!-- Title -->
            <text x="0" y="60" fill="#0F172A" font-size="26" font-weight="800" letter-spacing="-0.5">62.5시간 백테스트 누적 손익 곡선</text>
            <text x="0" y="82" fill="#64748B" font-size="13.5" font-weight="500">바이낸스 1초 리드래그 충격 감지 ➔ 하이퍼리퀴드 지정가(Post-Only) 메이커 체결 분석 (총 512만 틱)</text>
        </g>
    "##);

    // 2. 4-COLUMN KPI CARDS (y: 148..220)
    let kpi_y = 148.0;
    let kw = 260.0;
    let kh = 72.0;
    let kgap = 20.0;
    ...
    for (i, (label, val, sub, col)) in kpis.iter().enumerate() {
        ...
        svg.push_str(&format!(
            ...
            <g transform="translate({x}, {kpi_y})">
                ...
                <text x="16" y="22" fill="#64748B" font-size="10.5" font-weight="700" letter-spacing="0.5">{label}</text>
                <text x="16" y="48" fill="{col}" font-size="21" font-weight="800">{val}</text>
                <text x="16" y="64" fill="#94A3B8" font-size="10" font-weight="500">{sub}</text>
            </g>
            "##,
            x = x, kpi_y = kpi_y, cw = kw, ch = kh, label = label, val = val, sub = sub, col = col
        ));
    }

    // 3. MAIN CARD BASE RECT (with shadow)
    svg.push_str(&format!(
        r##"<rect x="{cx}" y="{cy}" width="{cw}" height="{ch}" rx="16" fill="#FFFFFF" stroke="#E2E8F0" stroke-width="1" filter="url(#softCardShadow)"/>"##,
        cx = card_x, cy = card_y, cw = card_w, ch = card_h
    ));

    // MAIN PLOT AREA (With generous vertical margins!)
    let p_left = card_x + 75.0;
    let p_right = card_x + card_w - 75.0;
    let p_w = p_right - p_left; // 950.0 width

    let p_top = card_y + 90.0;  // 90px from card top
    let p_bottom = card_y + card_h - 60.0; // 60px from card bottom
    let p_h = p_bottom - p_top; // 325.0 height
    ...
    // Max PnL set to 1250.0 -> peak 806.21 comfortably sits well below the top!
    let min_pnl = -300.0;
    let max_pnl = 1200.0;
    ...
    // Y Axis Grids
    for pnl_val in [-200, 0, 200, 400, 600, 800, 1000] {
        ...
        svg.push_str(&format!(
            ...
            <text x="{tx}" y="{ty}" fill="{fc}" font-size="11.5" font-weight="600" text-anchor="end">${v}</text>"##,
            x1 = p_left, x2 = p_right, y = y, sc = stroke_col, sw = stroke_w, d = dash,
            tx = p_left - 12.0, ty = y + 4.0, fc = font_col, v = pnl_val
        ));
    }

    // Paths inside Strictly Clipped Group!
    svg.push_str(r##"<g clip-path="url(#mainCardClip)">"##);
    ...
    svg.push_str("</g>"); // Close clipped group

    // Top Legend Badge inside Card
    svg.push_str(&format!(
        r##"
        <g transform="translate({lx}, {ly})">
            <rect width="450" height="32" rx="8" fill="#F8FAFC" stroke="#E2E8F0" stroke-width="1"/>
    ...
    for i in 0..=4 {
        ...
        svg.push_str(&format!(
            ...
            x = x, y1 = p_bottom, y2 = p_bottom + 5.0, ty = p_bottom + 20.0, dt = dt
        ));
    }

    // 4. BOTTOM HOURLY PNL CARD (y: 745..1155)
    let bot_x = 50.0;
    let bot_y = 745.0;
    let bot_w = 1100.0;
    let bot_h = 405.0;
    ...
    for (hour, (_trades, _wins, _gross, net_m)) in hourly {
        ...
        let scaled_h = (*net_m / 800.0) * 190.0;
