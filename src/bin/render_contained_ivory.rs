...
fn main() {
    ...
    let svg_path = "/Users/elias/apex-alpha/scratch/ivory_perfect_contained.svg";
    generate_contained_svg(&records, &hourly_pnl, svg_path);
    println!("✅ Generated 100% Contained Ivory SVG");
}

fn generate_contained_svg(
    records: &[TradeRecord],
    ...
    // 2. 4-COLUMN KPI CARDS (Perfect Fit from x=50 to x=1150)
    let kpi_y = 145.0;
    let card_w = 260.0;
    let card_h = 72.0;
    let card_gap = 20.0;
    ...
    // 3. MAIN PNL CHART CARD (Container x=50..1150, y=235..725)
    let card_x = 50.0;
    let card_y = 235.0;
    let card_w = 1100.0;
    let card_h = 490.0;
    ...
    // Plot inside Main Card with Generous Internal Padding (NEVER ESCAPES!)
    let p_left = card_x + 75.0;
    let p_right = card_x + card_w - 60.0;
    let p_w = p_right - p_left; // 965.0 width

    let p_top = card_y + 80.0; // Top padding 80px (plenty of room so line never touches top!)
    let p_bottom = card_y + card_h - 55.0; // Bottom padding 55px
    let p_h = p_bottom - p_top; // 355.0 height
    ...
    // Set Max Y to 1000.0 (Our peak is 806.21, nicely sitting at 80% height!)
    let min_pnl = -250.0;
    let max_pnl = 1000.0;
    ...
    // End point dots & value tag inside card
    ...
    // Top Legend Badge inside Card (Nicely placed at top-left)
    svg.push_str(&format!(
        r##"
    ...
    // 4. BOTTOM HOURLY PNL CARD (Container x=50..1150, y=745..1155)
    let bot_x = 50.0;
    let bot_y = 745.0;
    let bot_w = 1100.0;
    let bot_h = 405.0;
    ...
    let zero_h_y = 310.0; // Baseline zero line
    ...
    for (hour, (_trades, _wins, _gross, net_m)) in hourly {
        ...
        // Scale to max $800 so tallest bar (+521) sits at 65% height, plenty of room below title!
        let scaled_h = (*net_m / 750.0) * 200.0;
